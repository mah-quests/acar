...
$today=date('Y-m-d');
$candidates = $db->rawQuery("SELECT * FROM candidates WHERE id_number not in (SELECT id_number from time_in where check_in_date='$today')");
...
<div id="page-wrapper">
    <div class="row">
        ...
        <div class="col-lg-6">
            <h1 class="page-header">Employees NOT Checked In</h1>
        </div>
    </div>
        ...
        <tbody>
            <?php foreach ($candidates as $row) : ?>
                <tr>
                    ...
                    <td align="center">
                    <a href="check_in_confirmed.php?candidate_id=<?php echo $row['id'] ?>&operation=edit" class="btn btn-primary" style="margin-right: 4px;"><span class="glyphicon glyphicon-plus"></span></a></td>
    ...
    <div class="text-center">
        ...
        if ($total_pages > 1) {
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
                ...
                echo '<li' . $li_class . '><a href="add_not_check_in.php' . $http_query . '&page=' . $i . '">' . $i . '</a></li>';
            }
            echo '</ul></div>';
        }
        ?>
